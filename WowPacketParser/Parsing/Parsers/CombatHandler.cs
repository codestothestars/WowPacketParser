...
namespace WowPacketParser.Parsing.Parsers
{
    public static class CombatHandler
    {
        ...
        public static void HandleAttackerStateUpdate406(Packet packet)
        {
            ...
            attackData.SubDamageCount = packet.ReadByte("SubDmg");
            for (var i = 0; i < attackData.SubDamageCount; ++i)
            {
                ...
                if (hitInfo.HasAnyFlag(SpellHitInfo.HITINFO_PARTIAL_ABSORB | SpellHitInfo.HITINFO_FULL_ABSORB))
                    attackData.TotalAbsorbedDamage += (uint)packet.ReadInt32("Absorbed", i);

                if (hitInfo.HasAnyFlag(SpellHitInfo.HITINFO_PARTIAL_RESIST | SpellHitInfo.HITINFO_FULL_RESIST))
                    attackData.TotalResistedDamage += (uint)packet.ReadInt32("Resisted", i);
            }
