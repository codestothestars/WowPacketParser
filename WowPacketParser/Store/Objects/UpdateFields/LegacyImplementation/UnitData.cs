...
namespace WowPacketParser.Store.Objects.UpdateFields.LegacyImplementation
{
    public class UnitData : IUnitData
    {
        ...
        public UnitData(WoWObject obj, bool useOriginal)
        {
            Object = obj;
        ...
        public int[] ResistanceBuffModsPositive => UpdateFields.GetArray<UnitField, int>(UnitField.UNIT_FIELD_RESISTANCEBUFFMODSPOSITIVE, 7);
        public int[] ResistanceBuffModsNegative => UpdateFields.GetArray<UnitField, int>(UnitField.UNIT_FIELD_RESISTANCEBUFFMODSNEGATIVE, 7);
        ...
        public float MinDamage => UpdateFields.GetValue<UnitField, float>(UnitField.UNIT_FIELD_MINDAMAGE);

        public float MaxDamage => UpdateFields.GetValue<UnitField, float>(UnitField.UNIT_FIELD_MAXDAMAGE);

        public float MinOffHandDamage => UpdateFields.GetValue<UnitField, float>(UnitField.UNIT_FIELD_MINOFFHANDDAMAGE);

        public float MaxOffHandDamage => UpdateFields.GetValue<UnitField, float>(UnitField.UNIT_FIELD_MAXOFFHANDDAMAGE);

        public float MinRangedDamage => UpdateFields.GetValue<UnitField, float>(UnitField.UNIT_FIELD_MINRANGEDDAMAGE);

        public float MaxRangedDamage => UpdateFields.GetValue<UnitField, float>(UnitField.UNIT_FIELD_MAXRANGEDDAMAGE);

        public int AttackPower => UpdateFields.GetValue<UnitField, int>(UnitField.UNIT_FIELD_ATTACK_POWER);

        public int AttackPowerModPos
        {
            get
            {
                if (Enums.Version.UpdateFields.GetUpdateField(UnitField.UNIT_FIELD_ATTACK_POWER_MODS) > 0)
                {
                    uint raw = UpdateFields.GetValue<UnitField, uint>(UnitField.UNIT_FIELD_ATTACK_POWER_MODS);
                    return (int)(raw & 0x0000FFFF);
                }
                else
                {
                    return UpdateFields.GetValue<UnitField, int>(UnitField.UNIT_FIELD_ATTACK_POWER_MOD_POS);
                }
            }
        }
        ...
        public int AttackPowerModNeg
        {
            get
            {
                ...
                {
                    uint raw = UpdateFields.GetValue<UnitField, uint>(UnitField.UNIT_FIELD_ATTACK_POWER_MODS);
                    return (int)((raw & 0xFFFF0000) >> 16);
                }
                else
                {
                    return UpdateFields.GetValue<UnitField, int>(UnitField.UNIT_FIELD_ATTACK_POWER_MOD_NEG);
                }
            }
        }

        public float AttackPowerMultiplier => UpdateFields.GetValue<UnitField, float>(UnitField.UNIT_FIELD_ATTACK_POWER_MULTIPLIER);

        public int RangedAttackPower => UpdateFields.GetValue<UnitField, int>(UnitField.UNIT_FIELD_RANGED_ATTACK_POWER);

        public int RangedAttackPowerModPos
        {
            get
            {
                if (Enums.Version.UpdateFields.GetUpdateField(UnitField.UNIT_FIELD_RANGED_ATTACK_POWER_MODS) > 0)
                {
                    uint raw = UpdateFields.GetValue<UnitField, uint>(UnitField.UNIT_FIELD_RANGED_ATTACK_POWER_MODS);
                    return (int)(raw & 0x0000FFFF);
                }
                else
                {
                    return UpdateFields.GetValue<UnitField, int>(UnitField.UNIT_FIELD_RANGED_ATTACK_POWER_MOD_POS);
                }
            }
        }
        ...
        public int RangedAttackPowerModNeg
        {
            get
            {
                ...
                {
                    return UpdateFields.GetValue<UnitField, int>(UnitField.UNIT_FIELD_RANGED_ATTACK_POWER_MOD_NEG);
                }
            }
        }

        public float RangedAttackPowerMultiplier => UpdateFields.GetValue<UnitField, float>(UnitField.UNIT_FIELD_RANGED_ATTACK_POWER_MULTIPLIER);
        ...
        public IUnitData Clone() { return new UnitData(Object, UseOriginalData); }
    }
}
