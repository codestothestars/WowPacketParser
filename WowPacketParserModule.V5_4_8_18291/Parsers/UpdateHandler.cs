        ...
        private static MovementInfo ReadMovementUpdateBlock(Packet packet, WowGuid guid, object index)
        {
            ...
            if (isLiving)
            {
                if (hasTransportData) //96
                {
                    ...
                    moveInfo.TransportSeat = packet.ReadSByte("Transport Seat", index); //72
                    packet.ReadXORByte(transportGuid, 6);
                    packet.ReadXORByte(transportGuid, 2);
                    moveInfo.TransportTime = packet.ReadUInt32("Transport Time", index); //76
                ...
                if (moveInfo.HasSplineData) //344
                {
                    if (hasFullSpline) //336
                    {
                        ...
                        monsterMove.MoveTime = packet.ReadUInt32("Spline Full Time", index); //236
                ...
                if (hasFallData) //132
                {
                    ...
                    {
                        moveInfo.JumpSinAngle = packet.ReadSingle("Jump Sin Angle", index); //124
                        moveInfo.JumpHorizontalSpeed = packet.ReadSingle("Jump Horizontal Speed", index); //116
                        moveInfo.JumpCosAngle = packet.ReadSingle("Jump Cos Angle", index); //120

                    }
                    moveInfo.FallTime = packet.ReadUInt32("Jump Fall Time", index); //108
                    moveInfo.JumpVerticalSpeed = packet.ReadSingle("Jump Vertical Speed", index); //112
                }
                ...
                if (hasSplineElevation) //136
                    moveInfo.SplineElevation = packet.ReadSingle("Spline Elevation", index);//196
                ...
                if (hasPitch)
                    moveInfo.SwimPitch = packet.ReadSingle("Pitch", index); //104
            ...
            if (hasTransportPosition)
            {
                ...
                moveInfo.TransportSeat = packet.ReadSByte("Transport Seat", index); //400
                ...
                moveInfo.TransportTime = packet.ReadUInt32("Transport Time", index); //404
